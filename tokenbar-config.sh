...
# ---- the lead line -------------------------------------------------------
# A third line above everything, owned by one plugin. The default is auto: the one
# plugin that has written modes/<session_id>/<name>.lead this session gets the
# line, and none or several render nothing. Set it only to override that -- a name
# to pin one plugin, or empty to have no lead line at all.
#
...
#
# LEAD_PLUGIN=review   # auto is the default; LEAD_PLUGIN="" is off
# LEAD_STYLE=bar     # bar -> |REVIEW DRAFT ... ; badge -> [REVIEW:DRAFT] ...
# LEAD_TITLE=60      # cells the title column occupies; longer titles are cut
